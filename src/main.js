    ...
    if (!$한글스크립트[$계수기]) {
      ...
      if (!$메모리버퍼[$한글한글[0]]) {
        if ($한글한글[0] === '말하기' || $한글한글[0] === 'ㅁ') {
          ...
          $계수기 = ($한글스크립트.length)
          document.getElementById('in-debugTextbox').value = $출력
          ...
          if (!$비교되는자 || !$비교하는자 || !$연산자 || !$같다면여기로 || !$아니면여기로) {
            $출력 += '알수없는 문법! ' + ($계수기 + 1) + '번째 줄, "만약 <숫자/변수이름> 가 <숫자/변수이름> 보다 크다면 <이동할 줄> 아니면 <이동할 줄>"을 사용하세요'
          }
          if (!Number($비교되는자)) {
            if (!$메모리버퍼[$비교되는자]) {
              $출력 += '선언되지 않은 변수이름! (혹은 문법오류) ' + ($계수기 + 1) + '번째 줄, "만약 <숫자/변수이름> 가 <숫자/변수이름> 보다 크다면 <이동할 줄> 아니면 <이동할 줄>"을 사용하세요'
            } else {
              $비교되는자 = $메모리버퍼[$비교되는자]
            }
          }
          ...
          if ($연산자 === '같다면') {
            ...
            $출력 += '알수없는 연산자! ' + ($계수기 + 1) + '번째 줄, ' + $연산자 + ', 같다면/다르면/크다면/작다면/같거나크다면/같거나작다면 을 사용하세요'
            $계수기++
          }
        } else if ($한글한글[0] === '뛰어넘기' || $한글한글[0] === 'ㄸ') {
          let $얼마나뛰어넘기 = Number($한글한글[1])
          if (!$얼마나뛰어넘기) {
            $출력 += '알수없는 문법! ' + ($계수기 + 1) + '번째 줄, "뛰어넘기 <뛰어넘을 줄>"을 사용하세요'
            $계수기++
          } else {
            if (!Number($얼마나뛰어넘기)) {
              if (!$메모리버퍼[$얼마나뛰어넘기]) {
                $출력 += '선언되지 않은 변수이름! (혹은 문법오류) ' + ($계수기 + 1) + '번째 줄, "뛰어넘기 <뛰어넘을 줄>"을 사용하세요'
              } else {
                if (!Number($메모리버퍼[$얼마나뛰어넘기])) {
                  $출력 += '숫자가 아닌 변수! (혹은 문법오류) ' + ($계수기 + 1) + '번째줄, 숫자가 아닌 변수만큼 뛰어넘을수 없습니다'
                } else {
          ...
          if (!$변수이름 || !$변수값) {
            $출력 += '알수없는 문법! ' + ($계수기 + 1) + '번째 줄, "정하기 <변수이름> <변수 값>"을 사용하세요'
          } else {
            if ($변수이름.includes('$')) {
              $출력 += '쓰기권한 없음! ' + ($계수기 + 1) + '번째 줄, "$"문자로 시작되는 변수이름은 값을 정할 수 없습니다'
            } else {
              $메모리버퍼[$변수이름] = $변수값
            }
            $계수기++
        ...
        if ($한글한글[1] === '에') {
          ...
          if ($연산자 === '더하기') {
            ...
            $출력 += '알수없는 연산자! ' + ($계수기 + 1) + '번째 줄, ' + $연산자 + ', 더하기/빼기/곱하기/나누기 를 사용하세요'
            $계수기++
          }
        }
      }
